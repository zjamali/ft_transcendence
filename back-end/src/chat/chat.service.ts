...
export class ChatService {
  ...
  async addUserChatSocket(user_id: string, socket_id: string) {
    ...
    if (opened_sockets)
      GlobalService.UsersChatSockets.set(user_id, [...opened_sockets, socket_id]);
    else 
      GlobalService.UsersChatSockets.set(user_id, [socket_id]);
  }
  ...
  async removeUserChatSocket(user_id: string, socket_id: string) {
    ...
    GlobalService.UsersChatSockets.set(
      user_id,
      GlobalService.UsersChatSockets.get(user_id)?.filter((socket) => socket != socket_id),
    );
