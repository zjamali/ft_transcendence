								...
								<Link href={{pathname: 'users/[userId]',query: { userId: `${user.id}`}}} key={user.id}>
									<div className="wall-friend-card" key={user.id}>
										...
										<div>
											...
											<Button variant="outlined" size="small" color="error" sx={{width: 95, fontSize: 11}}startIcon={<PersonRemoveIcon sx={{width: 14}}/>} onClick={()=>{alert("si")}}>Unfriend</Button>
										</div>
									</div>
								</Link>
							);
